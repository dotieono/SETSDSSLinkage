...
namespace ClinicSystem
{
    public partial class MainForm : Form
    {
       ...
       // public MainForm() { }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (is_duplicate_person >= 1)
            {

                DialogResult dialogResult = MessageBox.Show("The name " + txtFirstname.Text + " " + txtMiddlename.Text + "  " + txtLastname.Text + " is shared among some existing records " + " Do you wish to review these clients?", "SETS Registry Details", MessageBoxButtons.YesNo);
                if (dialogResult == DialogResult.Yes)
                {
                   
                    //ClearFields();
                    //ReloadDSSLinkageWindow();
                    GeneratePotentialDuplicatePersons();
            ...
            {
                MessageBox.Show(" " +
                       "A registration date cannot be set to a future date", "SETS: Client registry", MessageBoxButtons.OKCancel,
       ...
       if (dpDob.Value.Date == DateTime.Now.Date)
          ...
          if (dpDob.Value.Date > dpRegDate.Value.Date)
          {
              MessageBox.Show(" " +
                     "A birthdate must preceed a registration date", "SETS: Client registry", MessageBoxButtons.OKCancel,
                          MessageBoxIcon.Information);
              dpRegDate.Focus();
              return;
          }
    ...
    MessageBoxIcon.Information);
        ...
        private void CheckDuplicatePerson()

        {
            ...
            // specify stored procedure to execute
            sqlcmd.CommandType = CommandType.StoredProcedure;
            sqlcmd.CommandText = "check_duplicate_person";

            sqlcmd.Parameters.AddWithValue("@firstname", txtFirstname.Text);
            sqlcmd.Parameters.Add("@middlename", SqlDbType.VarChar).Value = txtMiddlename.Text;
            sqlcmd.Parameters.Add("@lastname", SqlDbType.VarChar).Value = txtLastname.Text;
            sqlcmd.Parameters.Add("@pfile_ref", SqlDbType.VarChar).Value = txtFileRef.Text;
            sqlcmd.Parameters.Add("@mfname", SqlDbType.VarChar).Value = txtMFirstname.Text;
            sqlcmd.Parameters.Add("@mjname", SqlDbType.VarChar).Value = txtMMiddlename.Text;
            SqlDataReader sqlrdr = sqlcmd.ExecuteReader();//sqlrdr reads from the select from procedure
            while (sqlrdr.Read())
            {
                ...
                if (sqlrdr.IsDBNull(0) == false)
                {
                    is_duplicate_person = int.Parse(sqlrdr.GetValue(0).ToString());
                }
        ...
        private void GeneratePotentialDuplicatePersons()

        {
            ...
            SqlCommand sqlcmd = new SqlCommand("gen_duplicate_persons", con);
            sqlcmd.CommandType = CommandType.StoredProcedure;
            sqlcmd.Parameters.AddWithValue("@firstname", txtFirstname.Text);
            sqlcmd.Parameters.Add("@middlename", SqlDbType.VarChar).Value = txtMiddlename.Text;
            sqlcmd.Parameters.Add("@lastname", SqlDbType.VarChar).Value = txtLastname.Text;
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
               //make the data grid view visible in case a match is found
                if (dgvClientSearchResults.Visible == false) { dgvClientSearchResults.Visible = true; }
                if (pnlMatchNotFound.Visible == true) { pnlMatchNotFound.Visible = false; }
